...
void MidiKeyboardComponent::mouseDownFinishedOnKey(int midiNoteNumber)
{
    state.noteOff (midiChannel, midiNoteNumber);
}
        ...
        if (newNote != oldNoteDown)
        {
            if (oldNoteDown >= 0)
            {
                ...
                if (! mouseDownNotes.contains (oldNoteDown))
                    mouseDownFinishedOnKey(oldNoteDown);
            }
            ...
            if (newNote >= 0)
            {
                ...
                mouseDownStartedOnKey(newNote, mousePositionVelocity * velocity);
...
void StickyMidiKeyboardComponent::mouseDownFinishedOnKey (int midiNoteNumber)
{
    ...
    if (! stuckKeys.contains (midiNoteNumber))
        MidiKeyboardComponent::mouseDownFinishedOnKey(midiNoteNumber);
}
